...
export default function WorkoutPlanProgress({ plans }: Props) {
    ...
    const [expanded, setExpanded] = useState<number | null>(null); // for accordion
    ...
    return (
        <div className="space-y-6" ref={containerRef}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <h4 className="font-semibold text-base text-gray-700">Workout Plans & Progress</h4>
            ...
            {visiblePlans.length === 0 ? (
                <p className="text-gray-500 text-sm italic">No plans in this category.</p>
            ) : (
                <>
                    {/* Cards */}
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {visiblePlans.map((plan, idx) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    className="relative bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex flex-col justify-between"
                                >
                                    ...
                                    >
                                        <div className="flex items-center gap-2 text-gray-700">
                                            ...
                                            <p className="font-semibold text-black text-sm sm:text-base">{plan.title}</p>
                                        </div>
                                    ...
                                    {/* Accordion content */}
<div className={`${isExpanded || window.innerWidth >= 640 ? 'block' : 'hidden'} mt-2`}>
  <p className="text-sm text-gray-600 line-clamp-2">{plan.description}</p>
  <p
    className="text-xs text-gray-500 mt-2 cursor-help"
  ...
  {plan.exercises && plan.exercises.length > 0 ? (
    <div className="mt-4">
      <p className="text-sm font-medium text-gray-700 mb-1">Exercises:</p>
      <ul className="list-disc list-inside text-sm text-gray-800 space-y-1">
        {plan.exercises.map((ex: any, i: number) => (
          <li key={`${ex.name}-${ex.sets}-${ex.reps}-${i}`}>

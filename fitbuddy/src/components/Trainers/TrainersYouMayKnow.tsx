...
export default function TrainersYouMayKnow() {
        ...
        <div className="p-6 bg-white rounded-xl shadow w-full">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10">
                    {paginatedTrainers.length === 0 && <p className="col-span-full">No trainers found.</p>}
                    {paginatedTrainers.map((trainer) => (
                        <div
                            key={trainer.id}
                            className="relative group bg-gray-200 rounded-xl shadow-sm border aspect-square flex flex-col justify-between items-center p-4 text-center hover:shadow-md transition"
                        >
                            {/* Tooltip */}
                            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full w-40 bg-black text-white text-xs rounded-md p-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                                ...
                                {isTrainerRequested(trainer.id) && (
                                    <p>
                                        Status:{' '}
                                        <span className={
                                            getTrainerRequestStatus(trainer.id) === 'pending'
                                                ? 'text-yellow-400'
                            ...
                            {/* <img
    src={`https://api.dicebear.com/8.x/thumbs/svg?seed=${trainer.name}`}
    alt="Profile"
  ...
  /> */}
                            ...
                            <div className="mt-2">
                                <p className="font-semibold text-sm truncate text-black">{trainer.name}</p>
                                ...
                                {isTrainerRequested(trainer.id) ? (
                                    <span
                                        className={`px-2 py-1 text-xs rounded-md font-medium ${getTrainerRequestStatus(trainer.id) === 'pending'
                                                ? 'bg-yellow-100 text-yellow-800 px-7 py-2 mb-3'
                                                : getTrainerRequestStatus(trainer.id) === 'accepted'
  ...
  <h3 className="text-lg font-semibold text-black">Sent Requests</h3>
  <button
  ...
  <div className="space-y-3">
    {sentRequests.length === 0 ? (
      ...
      sentRequests.map((req) => (
        <div
          key={req.id}
          className="flex justify-between items-center bg-gray-100 p-3 rounded-md text-black font-semibold"
        >
          <div>
  ...
  className="bg-red-100 text-red-700 hover:bg-red-400 cursor-pointer hover:text-red-800 px-3 py-1 rounded-full text-sm font-medium transition-all shadow-sm"
>
  Cancel
</button>
            )}
